...
vector<pair<int, int>> getAntiNodes(int x1, int y1, int x2, int y2)
{
    ...
    while (firstNode_x >= 0 and firstNode_x < grid.size() and firstNode_y >= 0 and firstNode_y < grid[0].size())
    {
        antiNodes.push_back({firstNode_x, firstNode_y});
        firstNode_x += dx;
        firstNode_y += dy;
    }
    while (secondNode_x >= 0 and secondNode_x < grid.size() and secondNode_y >= 0 and secondNode_y < grid[0].size())
    {
